...
function Header() {
  ...
  return (
    <header className="py-3.5 dark:bg-slate-950 transition-all duration-300">
                ...
                <AnimatePresence>
                  {isOpen && (
                    <motion.div
                      ...
                      transition={{
                        duration: 0.5,
                        ease: [0.04, 0.62, 0.23, 0.98], // Silliq (cubic-bezier) poydevor
                      }}
                      className="absolute top-full right-0 z-10 bg-white dark:bg-slate-900 shadow-lg rounded-lg p-3.5 overflow-hidden"
                    >
                      <div className={`flex flex-col gap-3 transition-all duration-500 ${isOpen ? "opacity-100" : "opacity-0"}`}>
      ...
      <div className="w-full h-px bg-[#FEC80B] mt-2.5 mb-4"></div>
      <HeaderBottom />
    </header>
  );
}
